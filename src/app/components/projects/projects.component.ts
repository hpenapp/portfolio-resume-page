    ...
    <section id="projects" class="py-32 px-4 md:px-10 border-t border-white/5 relative overflow-hidden">
      <div class="max-w-6xl mx-auto relative z-10">
        <div class="mb-20 text-center" data-aos="fade-up">
          ...
          <p class="text-textSecondary text-base max-w-2xl mx-auto leading-relaxed">A selection of technical implementations ranging from data pipelines to AI-driven analytical tools.</p>
        </div>
        ...
        <div class="flex flex-wrap justify-center gap-3 mb-16" data-aos="fade-up">
          <button *ngFor="let filter of filters" 
                  ...
                  class="px-8 py-2.5 rounded-full border transition-all duration-300 font-bold text-sm hover:scale-105 active:scale-95 cursor-pointer backdrop-blur-md"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          <div *ngFor="let project of filteredProjects; let i = index" 
            ...
            <div class="bezel-inner glass-card flex flex-col h-full cursor-pointer transition-all duration-300 active:scale-[0.98] active:filter active:blur-[1px]">
              ...
              <div class="p-8 flex-grow flex flex-col">
                <div class="flex items-center gap-2 mb-4">
                  ...
                  <span *ngIf="project.client" class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.1em]">{{ project.client }}</span>
                </div>

                <h3 class="text-xl font-bold text-white mb-4 tracking-tight group-hover:text-primary transition-colors duration-500">{{ project.title }}</h3>
                ...
                <div class="flex flex-wrap gap-2">
                  <span *ngFor="let tech of project.technologies | slice:0:3" 
                        class="text-[11px] font-mono font-bold bg-primary/5 text-primary/70 px-3 py-1 rounded-md border border-primary/10 transition-colors group-hover:bg-primary/10 group-hover:text-primary">
                    {{ tech }}
                  </span>
      ...
      <div *ngIf="selectedProject" class="fixed inset-0 z-[60] flex items-center justify-center p-4 md:p-10 animate-fade-in">
        ...
        <div class="relative w-full max-w-5xl bg-background border border-white/10 rounded-[2.5rem] shadow-[0_0_100px_rgba(0,0,0,0.5)] overflow-hidden max-h-[90vh] flex flex-col animate-fade-in-up"
          ...
          <button (click)="closeModal()" class="absolute top-6 right-6 z-20 p-4 bg-white/5 hover:bg-white/10 text-white rounded-full transition-all active:scale-90 border border-white/10 backdrop-blur-md" aria-label="Close modal">
          ...
          <div class="h-64 md:h-96 relative flex-shrink-0 overflow-hidden">
              ...
              <div class="absolute bottom-8 left-8 md:left-12">
                ...
                <h3 class="text-3xl md:text-5xl font-bold text-white tracking-tighter">{{ selectedProject.title }}</h3>
              </div>
          </div>
          ...
          <div class="p-8 md:p-12 overflow-y-auto custom-scrollbar">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
              <div class="lg:col-span-2">
                ...
                <p class="text-gray-300 text-base leading-relaxed whitespace-pre-line mb-10 font-medium">
                  {{ selectedProject.longDescription || selectedProject.description }}
                </p>
                
                <h4 class="text-sm font-bold text-primary uppercase tracking-[0.2em] mb-6">Stack</h4>
                <div class="flex flex-wrap gap-3">
                  <span *ngFor="let tech of selectedProject.technologies" class="px-5 py-2.5 bg-primary/10 text-primary/80 rounded-xl text-sm font-bold border border-primary/20 hover:border-primary/40 transition-colors">
                    {{ tech }}
                  </span>
                </div>
              </div>
              ...
              <div class="space-y-8">
                <div *ngIf="selectedProject.client" class="p-6 bg-white/5 rounded-2xl border border-white/5">
                  <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mb-3">Client</h4>
                  <p class="text-white font-bold">{{ selectedProject.client }}</p>
                </div>

                <div class="flex flex-col gap-4 pt-6">
                  <a *ngIf="selectedProject.link && selectedProject.link !== '#'" [href]="selectedProject.link" target="_blank" class="w-full flex items-center justify-center gap-3 px-8 py-4 bg-primary text-background font-bold rounded-full hover:bg-cyan-300 transition-all active:scale-[0.97] shadow-lg">
                    <span>Live Demo</span>
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                  </a>
                  <a *ngIf="selectedProject.githubLink && selectedProject.githubLink !== '#'" [href]="selectedProject.githubLink" target="_blank" class="w-full flex items-center justify-center gap-3 px-8 py-4 bg-white/5 border border-white/10 text-white font-bold rounded-full hover:bg-white/10 transition-all active:scale-[0.97]">
